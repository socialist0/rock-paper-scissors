...
### 블록쌓기 게임 시작 불가 버그 (2026-06-06)
- **원인**: `blockAnimate()`가 일반 함수(`function`)로 선언되어 있는데 내부에서 `await blockGameOver()`를 호출하여 JS 문법 오류 발생. 파일 전체가 실행되지 않아 버튼 이벤트 등록 자체가 안 됨.
- **해결**: `await` 제거 후 `blockGameOverCalled` 플래그를 추가하여 게임오버 중복 호출 방지. `blockInitGame()` 시작 시 플래그 초기화.
- **교훈**: `requestAnimationFrame` 루프 함수는 `async`로 만들 수 없으므로 내부에서 `await` 사용 불가. 비동기 처리가 필요한 경우 플래그로 제어 후 일반 호출해야 함.
